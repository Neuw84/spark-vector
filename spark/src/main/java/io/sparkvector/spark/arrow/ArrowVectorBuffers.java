...
/**
 * {@link VectorBuffers} over an unshaded Arrow Java {@link ValueVector}, zero copy.
 *
 * <p>Two modes: {@link #forRead} drops the validity segment when the vector has no nulls so
 * kernels take their no-null fast path, while {@link #forWrite} always exposes a validity segment
 * so kernels can write validity bits directly into a freshly allocated vector.
 */
public final class ArrowVectorBuffers implements VectorBuffers {
  ...
  /** Maps a supported Arrow vector class to its physical type, or {@code null} if unsupported. */
  ...
  /**
   * Writable view over a vector that has already been sized with {@code allocateNew(length)} (or
   * {@code allocateNew(bytes, length)} for variable width). The caller writes values and validity
   * bits through the segments and then calls {@code vector.setValueCount(length)}.
   */
  ...
  private static ArrowVectorBuffers wrap(
      ValueVector v, VecType type, int n, MemorySegment validity) {
