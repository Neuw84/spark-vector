...
/**
 * A struct field of a type without a lane (struct, array, map, wide decimal), passed through as a
 * column of its own: the field's child vector with every ancestor struct's nulls folded in, since
 * Spark's {@code GetStructField} is null wherever the struct is. Children of the field share the same
 * ancestors, so a nested struct read through this view stays consistent. Owned by whoever owns the
 * wrapped vector; closing the view closes nothing.
 */
public final class NestedFieldColumnVector extends ColumnVector {
  ...
  /** Walks {@code path} down the struct children of {@code root}; the ancestors that carry nulls are kept for the null check. */
  ...
  /** A field of this struct field: the same ancestors apply, plus this level if it carries nulls. */
  @Override
