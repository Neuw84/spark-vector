...
/**
 * A dense view over a column the kernels cannot read (struct, array, map, wide decimal): row {@code i}
 * of the view is row {@code rows[i]} of the wrapped Spark vector. This is how a selection is applied to
 * a column that is passed through untouched -- our own columns are compacted into new Arrow vectors,
 * a foreign one keeps Spark's vector and remaps the row ids, children included. The wrapped vector
 * stays owned by whoever produced it (the child keeps it alive until its next batch), so closing the
 * view closes nothing.
 */
public final class RemappedColumnVector extends ColumnVector {
  ...
  /** The view of {@code cv} over the rows selected in the bitmap (the first {@code count} set bits of {@code numRows}). */
  ...
  /** The selected row ids of a bitmap, in order: the mapping every foreign column of a batch shares. */
  public static int[] rowsOf(MemorySegment selection, int numRows, int count) {
    int[] rows = new int[count];
    int k = 0;
    for (int i = 0; i < numRows && k < count; i++) {
