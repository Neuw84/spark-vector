 ...
 *
 * <p>A column may be <em>borrowed</em>: a projection that forwards an input column unchanged wraps
 * the child's vector without taking ownership, so closing the output batch leaves the child's
 * memory alone (the child releases it when it produces its next batch).
 */
