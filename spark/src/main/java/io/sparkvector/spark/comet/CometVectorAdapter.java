...
/**
 * Zero-copy view of Comet's scan output ({@code org.apache.comet.vector.CometVector}, backed by
 * Arrow buffers that Comet's native reader handed to the JVM through Arrow FFI).
 *
 * <p>Comet shades Arrow to {@code org.apache.comet.shaded.arrow}, so its vectors are not our
 * Arrow classes and this jar must not depend on Comet at compile time. Everything is resolved
 * reflectively once, on first use, and only the buffer addresses are read afterwards. Comet's
 * dictionary-encoded strings become dictionary-encoded UTF8 buffers, so group-by keys hash the
...
public final class CometVectorAdapter implements ColumnVectorAdapters.Adapter {
  ...
  /**
   * Registers the adapter if Comet is on the classpath. Safe to call repeatedly; returns whether
   * Comet vectors are now handled zero-copy.
   */
  public static synchronized boolean tryRegister() {
    ...
    try {
      ...
      LOG.warn("spark-vector: Comet found but its vector API differs from the expected one; " +
          "Comet batches will be copied instead of read zero-copy", e);
      registered = false;
    }
    return registered;
  }
  ...
  private VectorBuffers adaptDictionary(ColumnVector cv, int numRows, VecType type)
