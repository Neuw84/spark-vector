...
/**
 * Producer side of the <a href="https://arrow.apache.org/docs/format/CDataInterface.html">Arrow C
 * Data Interface</a>, written with the FFM API instead of {@code arrow-c-data}.
 *
 * <p>The interface is two C structs ({@code ArrowSchema}, {@code ArrowArray}) whose fields point at
 * the buffers, plus a release callback the consumer invokes when it is done. Filling the structs is
 * a few dozen stores; the callback is an FFM upcall stub. That is enough for another Arrow
 * implementation in the same process (Comet's shaded one) to wrap our buffers without copying, and
 * it needs neither {@code arrow-c-data} nor its JNI library on our side, which matters because Comet
 * ships that library's classes under their original names with shaded signatures.
 *
 * <p>Lifetime: an export retains the Arrow buffers it points at until the consumer's release call;
 * structs, strings and any copied buffers live in a GC-managed arena that becomes unreachable at
 * the same moment. Releases may arrive on any thread, including native ones, so the registry is
 * concurrent.
 */
public final class ArrowCData {

  /** ArrowArray struct: length, null_count, offset, n_buffers, n_children, buffers, children, dictionary, release, private_data. */
  static final long ARRAY_SIZE = 80;
  /** ArrowSchema struct: format, name, metadata, flags, n_children, children, dictionary, release, private_data. */
  ...
  static {
    try {
      ...
      RELEASE_ARRAY =
          linker.upcallStub(
              lookup.findStatic(ArrowCData.class, "releaseArray", MethodType.methodType(void.class, MemorySegment.class)),
              FunctionDescriptor.ofVoid(ValueLayout.ADDRESS),
              Arena.global());
      RELEASE_SCHEMA =
          linker.upcallStub(
              lookup.findStatic(ArrowCData.class, "releaseSchema", MethodType.methodType(void.class, MemorySegment.class)),
              FunctionDescriptor.ofVoid(ValueLayout.ADDRESS),
              Arena.global());
  ...
  /** One exported column: its arena (structs, strings, pointer arrays, any copied buffers) and retained Arrow buffers. */
  ...
  /**
   * Exports {@code in} as the C Data structs for a column named {@code name} of Spark type {@code
   * dt}. Buffers of our own Arrow vectors are pointed at directly (and retained); anything else is
   * copied into the export's arena. Dictionary-encoded strings are decoded (the consumer would
   * decode them anyway).
   */
  public static Exported export(VectorBuffers in, String name, DataType dt) {
    ...
    try {
      ...
      if (in instanceof ArrowVectorBuffers a) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          if (!in.isNull(i)) {
            ...
            MemorySegment.copy(dict.data(), ValueLayout.JAVA_BYTE, start, data, ValueLayout.JAVA_BYTE, pos, len);
            pos += len;
          }
        }
        ...
        if (in.type() == VecType.UTF8) {
          ...
          MemorySegment.copy(in.offsets(), 0, offsets, 0, ((long) n + 1) << 2);
      ...
      buffers.setAtIndex(I64, 0, validity == null ? 0L : validity.address());
      if (offsets != null) {
  ...
  /** {@code void release(struct ArrowArray*)}: called by the consumer, possibly from a native thread. */
  ...
  /** {@code void release(struct ArrowSchema*)}: the schema owns nothing beyond the export's arena. */
