...
/**
 * Zero-copy adapter for the batches Iceberg's own JVM vectorized Parquet reader produces (Spark's
 * {@code BatchScanExec} over an Iceberg table when Comet's native Iceberg scan is absent or falls
 * back, e.g. Comet 1.0 on format version 3 tables).
 *
 * <p>Two things differ from Comet's vectors. Iceberg does not maintain Arrow validity buffers (it
 * disables Arrow's null checking and tracks nulls in a byte-per-row {@code NullabilityHolder}), so
 * the validity bitmap is derived from that array into the batch's scratch arena while the data
 * and offset buffers are wrapped in place. And on merge-on-read tables the reader applies deletes
 * by wrapping every column in a {@code ColumnVectorWithFilter} that remaps row ids through a
 * shared {@code int[] rowIdMapping} and reports the live row count as the batch size.
 * {@link #normalize} unwraps such a batch into the delegate vectors plus a
 * {@link SelectedColumnarBatch} whose selection has one bit per live position, so the operators
 * evaluate over the physical batch with the selection they already understand, instead of paying
 * the indirection on every access.
 *
 * <p>Everything is resolved reflectively; the plugin has no compile-time Iceberg dependency and
 * the adapter registers only when the Iceberg runtime is on the classpath. Any vector it does not
 * recognise (constant partition columns, dictionary-encoded non-string columns, oversized
 * dictionaries) is declined to the copying path, which reads through Iceberg's accessors and is
 * always correct.
 */
public final class IcebergVectorAdapter implements ColumnVectorAdapters.Adapter {
  ...
  private static final String ARROW_COLUMN_VECTOR =
      "org.apache.iceberg.spark.data.vectorized.IcebergArrowColumnVector";
  private static final String COLUMN_VECTOR_WITH_FILTER =
  ...
  private static final String SHADED_VALUE_VECTOR =
      "org.apache.iceberg.shaded.org.apache.arrow.vector.ValueVector";
  ...
  private static final String SHADED_PARQUET_DICTIONARY =
      "org.apache.iceberg.shaded.org.apache.parquet.column.Dictionary";
  ...
  /**
   * Registers the adapter if the Iceberg Spark runtime is on the classpath. Safe to call
   * repeatedly; returns whether Iceberg batches are now handled zero-copy.
   */
  public static synchronized boolean tryRegister() {
    ...
    try {
      ...
      LOG.warn("spark-vector: Iceberg found but its vectorized reader differs from the expected one; " +
          "Iceberg batches will be copied instead of read zero-copy", e);
      registered = false;
    }
    return registered;
  }
  ...
  /**
   * If {@code batch} is an Iceberg merge-on-read batch whose columns are row-id-mapped, returns a
   * {@link SelectedColumnarBatch} over the unwrapped delegate vectors (physical row count, the
   * live positions selected, columns not owned); otherwise returns {@code batch} itself. The
   * caller closes the returned batch when it differs from the input (this releases only the
   * selection bitmap; Iceberg keeps owning the vectors).
   */
  ...
  private MemorySegment validityOf(Object holder, int numRows, Arena scratch)
  ...
  private VectorBuffers wrap(Object vector, int numRows, VecType type, MemorySegment validity)
      throws ReflectiveOperationException {
   ...
   * the Parquet dictionary is decoded once per batch into the scratch arena. Declined when the
   * dictionary is larger than the batch, where decoding every entry would cost more than the copy
   * path's per-row decode.
   */
  private VectorBuffers adaptDictionaryUtf8(
