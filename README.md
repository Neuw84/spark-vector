...
| Predicates | `=`, `!=`, `<`, `<=`, `>`, `>=` on numeric/date/decimal/string columns vs literal or column (strings in `UTF8_BINARY` order, once per dictionary entry on dictionary pages); `IN (literals)`; `LIKE 'p%'` / `'%p'` / `'%p%'` and `startswith` / `endswith` / `contains` against a literal; `InSet` (long `IN` lists, one binary search per row); `<=>`; `isnan`; `BETWEEN`; `AND`/`OR`/`NOT`; `IS [NOT] NULL`; boolean columns and their comparisons | `LIKE` with inner wildcards or `_`, an `IN` set holding `NULL`, functions |
