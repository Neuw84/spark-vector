...
| `spark.vector.exec.sortMergeJoin.mode` | `off` (default), `hash` (the rewrite above; what the boolean flag means) or `merge`: plan our order-preserving merge join over Spark's sorted inputs (#286) -- every join type, no statistics needed, Spark's row order kept. `auto` is #287. |
