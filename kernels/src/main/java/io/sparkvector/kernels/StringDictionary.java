...
/**
 * Distinct UTF8 values of any length, stored contiguously so the dictionary reads as a UTF8 VectorBuffers:
 * the group table's per-key dictionary and the shuffle writer's staging dictionary (#377). Each entry is
 * keyed by its length and a 64-bit fingerprint (see PACKED_KEY_BYTES).
 */
public final class StringDictionary {

  /**
   * Values of up to this many bytes are keyed by their packed bytes (so equality is a long compare);
   * longer values by a 64-bit hash plus a byte compare.
   */
  ...
  public int indexOf(long packed, int len, byte[] src, int from, boolean insert) {
    if (len == 1) {
  ...
  /** Per-caller byte scratch for {@link #indexOf(MemorySegment, long, int, boolean, Scratch)}. */
  public static final class Scratch {
    byte[] bytes = new byte[64];
  }

  /** {@link #indexOf} over a value that lives in a segment (an input dictionary's entry). */
  public int indexOf(MemorySegment data, long start, int len, boolean insert, Scratch s) {
  ...
  /** Forgets every value: ids start again from 0 (the shuffle writer, once every pending row is flushed). */
  public void clear() {
    Arrays.fill(ids, -1);
    Arrays.fill(singleByte, -1);
    size = 0;
