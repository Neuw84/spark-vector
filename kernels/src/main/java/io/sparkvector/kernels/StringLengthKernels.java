...
/**
 * The string functions computable from the offsets and a scan of the bytes alone: {@code length}
 * (code points), {@code octet_length} / {@code bit_length} (bytes), {@code ascii} (the first code
 * point) -- all INT32 out -- and {@code chr}, the one that writes a string.
 *
 * <p>Dictionary-encoded input is handled the way every string function should: the per-string value
 * is computed once per dictionary entry, then gathered through the indices ({@link #perEntry}).
 * Code points are counted with Spark's {@code UTF8String.numChars} walk over its first-byte width
 * table ({@link StringSliceKernels#numBytesForFirstByte}), so malformed input counts exactly as Spark
 * counts it. Null rows (validity clear) get 0 in an INT32 output and no bytes in a string output; the
 * caller supplies the validity.
 */
public final class StringLengthKernels {
  ...
  private static int measure(Measure m, MemorySegment data, long start, int len) {
    switch (m) {
      case CHARS:
  ...
  /**
   * Spark's {@code ascii}: the code point of {@code substring(0, 1).toString()}, which decodes the
   * first sequence as Java does -- a lone continuation byte, a disallowed lead byte, a truncated or
   * ill-formed sequence gives U+FFFD.
   */
  static int firstCodePoint(MemorySegment data, long start, int len) {
    ...
    if (nb == 1 || nb > len) return REPLACEMENT;
    int cp = b0 & (0xFF >> (nb + 1));
    for (int k = 1; k < nb; k++) {
      int b = data.get(ValueLayout.JAVA_BYTE, start + k) & 0xFF;
      if ((b & 0xC0) != 0x80) return REPLACEMENT;
      cp = (cp << 6) | (b & 0x3F);
    }
    // Overlong forms, surrogates and values past U+10FFFF are what the decoder rejects.
    if ((nb == 2 && cp < 0x80) || (nb == 3 && (cp < 0x800 || (cp >= 0xD800 && cp <= 0xDFFF))) || (nb == 4 && (cp < 0x10000 || cp > 0x10FFFF))) {
      return REPLACEMENT;
    }
    return cp;
  }

  /** {@code m} over every string of {@code v} into an INT32 lane; dictionary input once per entry. */
  public static SegmentVectorBuffers measure(Measure m, VectorBuffers v, MemorySegment validity, Arena arena) {
    int n = v.length();
    ...
    if (dict != null) {
      ...
      for (int i = 0; i < n; i++) {
        if (validity == null || Bitmap.isSet(validity, i)) out.setAtIndex(VectorBuffers.LE_INT, i, perEntry[v.getInt(i)]);
      }
    } else {
  ...
  /**
   * Spark's {@code chr(n)} over an INT32 or INT64 lane: a negative {@code n} gives {@code ''}, the
   * low byte {@code n & 0xFF} is the character -- 0 is the NUL character, 1..127 one byte, 128..255
   * the two-byte UTF-8 encoding of that Latin-1 code point.
   */
