...
public final class DateKernels {
  ...
  /** {@code last_day}: the last day of the date's month, as days since the epoch. */
  ...
  /** {@code add_months}: {@code java.time}'s plusMonths -- the day clamped to the target month's length. */
  ...
  /** Spark's {@code getDayOfWeekFromString} code: SU=3 MO=4 TU=5 WE=6 TH=0 FR=1 SA=2 (1970-01-01 was a Thursday), -1 when unknown. */
  public static int dayOfWeekCode(String name) {
    switch (name.toUpperCase(java.util.Locale.ROOT)) {
      case "SU": case "SUN": case "SUNDAY": return 3;
      case "MO": case "MON": case "MONDAY": return 4;
      case "TU": case "TUE": case "TUESDAY": return 5;
      case "WE": case "WED": case "WEDNESDAY": return 6;
      case "TH": case "THU": case "THURSDAY": return 0;
      case "FR": case "FRI": case "FRIDAY": return 1;
      case "SA": case "SAT": case "SATURDAY": return 2;
      default: return -1;
    }
  }
  ...
  /** {@code make_date}: days since the epoch, or {@code Integer.MIN_VALUE} when the civil date is invalid. */
  public static int makeDate(int year, int month, int day) {
    if (year < -999999999 || year > 999999999 || month < 1 || month > 12 || day < 1 || day > lengthOfMonth(year, month)) return Integer.MIN_VALUE;
    ...
    return epochDay < Integer.MIN_VALUE + 1 || epochDay > Integer.MAX_VALUE ? Integer.MIN_VALUE : (int) epochDay;
  }

  private static final long SECONDS_PER_DAY = 86_400L;

  /**
   * {@code months_between} as Spark's {@code DateTimeUtils.monthsBetween} over two instants under a
   * fixed offset: whole months when the days of month agree or both are month ends, otherwise the
   * day-and-second difference over a 31-day month, rounded to 8 places when {@code roundOff}.
   */
  public static double monthsBetween(long micros1, long micros2, boolean roundOff, long offsetMicros) {
    ...
    if (d1 == d2 || (toEnd1 == 0 && toEnd2 == 0)) return monthDiff;
