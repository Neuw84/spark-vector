...
/**
 * Word-at-a-time logic over bitmaps: boolean algebra, Spark's three-valued AND/OR, null tests and
 * selection masks. A {@code null} validity segment means "all valid" throughout.
 *
 * <p>These deliberately use 64-bit scalar words rather than {@code LongVector}: bitmaps are 1/32 to
 * 1/64 the size of the data they describe, and validity segments coming from Arrow are not
 * guaranteed to be padded to a full SIMD register, so the tail handling would cost more than the
 * vectorization saves.
 */
public final class BitmapKernels {
  ...
  /** {@code a & ~b}. */
  public static void andNot(MemorySegment a, MemorySegment b, MemorySegment out, int n) {
    int words = Bitmap.wordsFor(n);
    for (int w = 0; w < words; w++) {
      Bitmap.setWord(out, w, n, Bitmap.wordAt(a, w, n) & ~Bitmap.wordAt(b, w, n));
    }
  ...
  /**
   * Validity of a result that is null whenever any operand is null: the AND of the operands'
   * validity bitmaps. Returns {@code false} and leaves {@code out} untouched if neither operand has
   * nulls (the caller then uses a {@code null} validity).
   */
  public static boolean combineValidity(
      MemorySegment aValid, MemorySegment bValid, MemorySegment out, int n) {
  ...
  /** Selection mask: element is true and not null (nulls are treated as false, as in WHERE). */
  ...
  /** {@code IS NULL}: result bits are the inverted validity; the result itself is never null. */
  ...
  /**
   * Three-valued AND. {@code false AND null = false}; {@code true AND null = null}. Output validity
   * is always written.
   */
