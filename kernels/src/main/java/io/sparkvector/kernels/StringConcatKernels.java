...
/**
 * {@code concat}, {@code concat_ws} and {@code elt}: string functions over <em>several</em> inputs.
 * Each row's output length is a sum across the inputs, so the offsets are the prefix sum of those
 * sums and the second pass is a per-input loop over byte ranges -- nothing per character. An input is
 * a UTF8 lane (dictionary-encoded read through the dictionary) or a literal's bytes
 * ({@link Part}). Semantics are Spark's {@code UTF8String.concat} / {@code concatWs} and
 * {@code Elt}: {@code concat} is null when any input is null; {@code concat_ws} is null only for a
 * null separator, skips null inputs and separates the live ones; {@code elt} picks input
 * {@code index} (1-based) and is null for a null index, an out-of-range index or a null pick. The
 * per-batch output is capped at {@link StringSliceKernels#MAX_OUTPUT_BYTES}.
 */
public final class StringConcatKernels {
  ...
  public static final class Part {
    ...
    private Part(VectorBuffers lane, byte[] literal) {
      this.lane = lane;
      this.store = lane == null ? null : (lane.dictionary() != null ? lane.dictionary() : lane);
    ...
    int length(int i) {
      if (lane == null) return (int) literal.byteSize();
    ...
    int copy(int i, MemorySegment out, long at) {
      ...
      MemorySegment.copy(store.data(), StringSliceKernels.startOf(store, idx), out, at, len);
      return len;
    }
  }
  ...
  public static SegmentVectorBuffers concat(Part[] parts, int n, MemorySegment validity, Arena arena) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      long sum = 0;
      for (Part p : parts) sum += p.length(i);
      lengths[i] = sum;
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (Part p : parts) at += p.copy(i, out, at);
    }
    return SegmentVectorBuffers.utf8(n, validity, offsets, out);
  }

  /**
   * {@code concat_ws(sep, parts...)}: the output validity is the separator's; a null part is skipped
   * and the separator sits only between the live parts.
   */
  public static SegmentVectorBuffers concatWs(Part sep, Part[] parts, int n, Arena arena) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (liveParts > 1) sum += (long) sep.length(i) * (liveParts - 1);
      lengths[i] = sum;
    }
    MemorySegment offsets = offsetsOf(lengths, n, arena);
    MemorySegment out = ArrowLayout.allocateBytes(arena, offsets.getAtIndex(VectorBuffers.LE_INT, n));
  ...
  /** The first row of {@code active} whose live index is outside {@code 1..count}, or -1. */
  public static int firstInvalidIndex(VectorBuffers index, int count, MemorySegment active, int n) {
    for (int i = 0; i < n; i++) {
      if (active != null && !Bitmap.isSet(active, i)) continue;
      if (index.validity() != null && !Bitmap.isSet(index.validity(), i)) continue;
  ...
  /** {@code elt(index, parts...)} with its own validity: null index, out-of-range index or null pick. */
  public static SegmentVectorBuffers elt(VectorBuffers index, Part[] parts, int n, Arena arena) {
    ...
    for (int i = 0; i < n; i++) {
      if (index.validity() != null && !Bitmap.isSet(index.validity(), i)) continue;
      int k = index.getInt(i);
      if (k <= 0 || k > parts.length) continue;
      Part p = parts[k - 1];
      if (!p.live(i)) continue;
