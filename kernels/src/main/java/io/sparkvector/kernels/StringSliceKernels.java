...
/**
 * String functions that <em>write</em> new UTF8 data: {@code substring}, {@code lpad}/{@code rpad},
 * {@code repeat}, {@code space} and {@code overlay}. Every kernel runs in two passes -- the first
 * computes each row's output byte length (and, for the slices, where its bytes come from), the second
 * writes into a data buffer sized once from the prefix sum of those lengths -- and returns a fresh
 * UTF8 {@link SegmentVectorBuffers} carrying the caller's validity.
 *
 * <p>Semantics are Spark's {@code UTF8String} to the letter: positions are 1-based code points,
 * {@code substringSQL}'s negative start counts from the end and an out-of-range range clamps silently
 * ({@link #substringRange}); code point widths come from the same first-byte table
 * ({@link #numBytesForFirstByte}); padding truncates to {@code len} code points when the string is
 * long enough and repeats the pad string, plus a partial one, otherwise. Dictionary-encoded input is
 * read through the dictionary; the output is always plain.
 *
 * <p>Integer arguments are either a scalar or an INT32 lane ({@code col == null} means scalar). Null
 * rows (validity clear) produce no bytes; the caller supplies the combined validity. The total output
 * is capped at {@link #MAX_OUTPUT_BYTES} per batch: past it the kernel refuses rather than allocate
 * unbounded native memory (a {@code repeat} or {@code space} with a runaway argument).
 */
public final class StringSliceKernels {
  ...
  /** Bytes of the code point starting with {@code b}, as Spark's {@code UTF8String.numBytesForFirstByte}. */
  ...
  /** Byte offset (within the string) after {@code chars} code points, clamped to {@code len}. */
  static int byteOffsetOfChar(MemorySegment data, long start, int len, int chars) {
    int i = 0;
    int c = 0;
    while (i < len && c < chars) {
      i += numBytesForFirstByte(data.get(ValueLayout.JAVA_BYTE, start + i));
  ...
  /**
   * Spark's {@code substringSQL(pos, length)} then {@code substring(start, until)} over the string at
   * {@code start}/{@code len}: returns the byte range as {@code (offset << 32) | byteLength}, both
   * relative to the string.
   */
  public static long substringRange(MemorySegment data, long start, int len, int pos, int length) {
    int chars = pos < 0 ? numChars(data, start, len) : 0;
    int from = pos > 0 ? pos - 1 : (pos < 0 ? chars + pos : 0);
    long untilL = (long) from + length;
    int until = untilL > Integer.MAX_VALUE ? Integer.MAX_VALUE : (untilL < Integer.MIN_VALUE ? Integer.MIN_VALUE : (int) untilL);
    if (until <= from || from >= len) return 0L;
    // Walk to `from` code points (a negative `from` walks nothing, as Spark's loop), then to `until`.
    int i = 0;
    int c = 0;
    while (i < len && c < from) { i += numBytesForFirstByte(data.get(ValueLayout.JAVA_BYTE, start + i)); c++; }
    int j = i;
    if (until == Integer.MAX_VALUE) {
      i = len;
    } else {
      while (i < len && c < until) { i += numBytesForFirstByte(data.get(ValueLayout.JAVA_BYTE, start + i)); c++; }
    }
    i = Math.min(i, len);
  ...
  private static void fillRepeated(MemorySegment out, long at, MemorySegment src, long srcStart, int srcLen, int times) {
  ...
  /** {@code substring(str, pos, len)}: a scalar or INT32 lane for each of {@code pos} and {@code len}. */
  public static SegmentVectorBuffers substring(
      VectorBuffers str, VectorBuffers posCol, int pos, VectorBuffers lenCol, int len,
  ...
  /**
   * {@code lpad} / {@code rpad(str, len, pad)}: {@code padCol} is a UTF8 lane or {@code null} with
   * {@code padBytes} the literal pad. Spark's rule: a non-positive {@code len} gives the empty string;
   * a string of at least {@code len} code points is cut to {@code len}; otherwise whole copies of the
   * pad and then a prefix of it fill the gap (no padding at all when the pad is empty).
   */
  public static SegmentVectorBuffers pad(
      VectorBuffers str, VectorBuffers lenCol, int len, VectorBuffers padCol, byte[] padBytes, boolean left,
      MemorySegment validity, Arena arena) {
    int n = str.length();
    VectorBuffers s = store(str);
    MemorySegment data = s.data();
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (spaces <= 0 || pLen == 0) {
        ...
        if (total > Integer.MAX_VALUE) throw new IllegalStateException("padded string exceeds the maximum length");
        outLen[i] = (int) total;
      }
    }
    MemorySegment offsets = ArrowLayout.allocateOffsets(arena, n);
  ...
  public static SegmentVectorBuffers repeat(
      VectorBuffers str, VectorBuffers timesCol, int times, MemorySegment validity, Arena arena) {
    int n = str.length();
    VectorBuffers s = store(str);
    MemorySegment data = s.data();
    int[] outLen = new int[n];
    ...
    for (int i = 0; i < n; i++) {
      ...
      int sLen = lengthOf(s, idx);
      if (t <= 0 || sLen == 0) continue;
      long total = (long) sLen * t;
      if (total > MAX_OUTPUT_BYTES) throw new IllegalStateException("repeat output of more than " + MAX_OUTPUT_BYTES + " bytes");
      count[i] = t;
      outLen[i] = (int) total;
    }
    MemorySegment offsets = ArrowLayout.allocateOffsets(arena, n);
    ...
    for (int i = 0; i < n; i++) {
      if (outLen[i] == 0) continue;
      int idx = index(str, i);
      fillRepeated(out, offsets.getAtIndex(VectorBuffers.LE_INT, i), data, startOf(s, idx), lengthOf(s, idx), count[i]);
    }
    return SegmentVectorBuffers.utf8(n, validity, offsets, out);
  }

  /** {@code space(n)}: {@code n} spaces, none for a negative count. */
  public static SegmentVectorBuffers space(int n, VectorBuffers countCol, int count, MemorySegment validity, Arena arena) {
    int[] outLen = new int[n];
    for (int i = 0; i < n; i++) {
      if (!live(validity, i)) continue;
  ...
  /**
   * {@code overlay(input, replace, pos, len)}: Spark's {@code Overlay.calculate} -- the first
   * {@code pos - 1} code points, the replacement, then the input from code point {@code pos + length}
   * where {@code length} is {@code len} when non-negative and the replacement's code point count
   * otherwise. {@code replCol} is a UTF8 lane or {@code null} with {@code replBytes} the literal.
   */
  public static SegmentVectorBuffers overlay(
      VectorBuffers input, VectorBuffers replCol, byte[] replBytes, VectorBuffers posCol, int pos,
      VectorBuffers lenCol, int len, MemorySegment validity, Arena arena) {
    int n = input.length();
    ...
    for (int i = 0; i < n; i++) {
      ...
      long total = (long) (int) head[i] + rLen + (int) tail[i];
      if (total > Integer.MAX_VALUE) throw new IllegalStateException("overlay output exceeds the maximum length");
      outLen[i] = (int) total;
    }
    MemorySegment offsets = ArrowLayout.allocateOffsets(arena, n);
    long total = prefix(outLen, offsets, n);
    ...
    for (int i = 0; i < n; i++) {
      ...
      at += rLen;
      MemorySegment.copy(data, sStart + (tail[i] >>> 32), out, at, (int) tail[i]);
    }
    return SegmentVectorBuffers.utf8(n, validity, offsets, out);
  }
}
