...
/**
 * The rounding family -- {@code ceil} / {@code floor}, {@code rint}, {@code round} (half up) and
 * {@code bround} (half even), with and without a scale -- over FLOAT64, INT32 / INT64 and decimal
 * (unscaled INT64) lanes. None of the kernels touches validity; null lanes hold arbitrary values.
 *
 * <p>Every definition is Spark's, not {@code Math.round}'s:
 *
 * <ul>
 *   <li>{@code ceil} / {@code floor} of a double is {@code (long) Math.ceil(d)}, so NaN is 0 and the
 *       infinities saturate exactly as Java's cast does;
 *   <li>{@code round(d, k)} / {@code bround(d, k)} of a double is {@code
 *       BigDecimal(Double.toString(d)).setScale(k, mode).doubleValue()} -- the SHORTEST decimal
 *       representation is rounded, which is why {@code round(2.675, 2)} is 2.68 although the binary
 *       value is below the half; NaN and the infinities pass through. This is inherently a scalar
 *       path;
 *   <li>rounding an integer to a negative scale is {@code BigDecimal(v).setScale(k, mode)} narrowed
 *       back: the exact result may not fit, and the caller either raises (ANSI, the lane is flagged
 *       in a mask) or keeps the wrapped low bits like {@code BigDecimal.intValue()} /
 *       {@code longValue()};
 *   <li>rounding a decimal is an unscaled-value operation: divide by a power of ten with the
 *       rounding mode, then (for a negative scale) multiply the integer result back up.
 * </ul>
 */
public final class RoundKernels {
  ...
  /** {@code out[i] = Math.rint(a[i])}: the nearest integral double, ties to even. */
  ...
  /** {@code out[i] = round(a[i], scale)} in {@code mode} as Spark's {@code RoundBase} does for doubles. */
  public static void roundDouble(VectorBuffers a, int scale, Mode mode, MemorySegment out) {
  ...
  /**
   * {@code out[i] = round(a[i], -power)} for INT32 / INT64 lanes (same lane type out), {@code power >= 1}:
   * the value rounded to a multiple of {@code 10^power} in {@code mode}. Where the exact result does
   * not fit the lane, {@code overflowBits} is set and the lane holds the wrapped low bits, like
   * {@code BigDecimal.intValue()} / {@code longValue()}. The mask must be cleared by the caller.
   */
  public static void roundIntegral(VectorBuffers a, int power, Mode mode, MemorySegment out, MemorySegment overflowBits) {
    int n = a.length();
    MemorySegment data = a.data();
  ...
  /**
   * Decimal rounding on unscaled INT64 lanes: {@code out[i] = roundDiv(a[i], 10^divPower, mode) *
   * 10^mulPower}. A decimal(p, s) rounded to scale {@code k >= 0} has {@code divPower = s - min(s, k)}
   * and {@code mulPower = 0}; to a negative scale {@code k} it has {@code divPower = s - k} and
   * {@code mulPower = -k} (result scale 0). {@code ceil} / {@code floor} are {@code divPower = s}.
   * {@code divPower} may exceed 18 (the divisor is then above every unscaled value, see
   * {@link #roundDiv}). The caller has already checked that the result type holds the exact result,
   * which Spark's result precision guarantees.
   */
  public static void roundDecimal(VectorBuffers a, int divPower, int mulPower, Mode mode, MemorySegment out) {
  ...
  /**
   * {@code v / 10^power} rounded in {@code mode}. For {@code power > 18} the divisor is above twice any
   * long's magnitude, so the half modes give 0 and the directed modes give the sign's unit.
   */
  ...
  /** {@link #roundDiv} for a divisor above twice any long's magnitude: the quotient is 0 or the sign's unit. */
