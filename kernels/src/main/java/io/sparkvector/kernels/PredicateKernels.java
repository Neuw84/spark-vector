...
/**
 * Predicates the comparison kernels do not cover: {@code isnan}, membership in a literal set
 * ({@code InSet}, the optimizer's rewrite of a long {@code IN} list) and comparisons between BOOL
 * columns, whose values are already packed bitmaps. Every kernel writes a result bitmap over all
 * {@code n} rows and leaves validity to the caller; null lanes hold arbitrary values.
 */
public final class PredicateKernels {
  ...
  /** {@code out[i] = isNaN(a[i])} for FLOAT64 lanes, as a Vector API {@code IS_NAN} test packed into the bitmap. */
  public static void isNaN(VectorBuffers a, MemorySegment out) {
    ...
    for (int w = 0; w < words; w++) {
      ...
      for (; i + lanes <= end; i += lanes) {
        long bits = DoubleVector.fromMemorySegment(D, data, (long) i << 3, LE).test(VectorOperators.IS_NAN).toLong();
        word |= bits << (i - base);
      }
      if (i < end) {
        VectorMask<Double> m = D.indexInRange(i, end);
        long bits = DoubleVector.fromMemorySegment(D, data, (long) i << 3, LE, m).test(VectorOperators.IS_NAN).and(m).toLong();
        word |= bits << (i - base);
      }
  ...
  /**
   * {@code out[i] = a[i] in keys} for INT32 / INT64 / FLOAT64 lanes. {@code keys} is sorted and
   * holds the set's values widened to longs -- for FLOAT64 the {@link Double#doubleToLongBits}
   * images, so membership is boxed {@code Double.equals}: NaN is a member of a set holding NaN and
   * {@code -0.0} is not a member of a set holding {@code 0.0}, exactly as Spark's {@code InSet}
   * over its {@code Set[Any]}. One binary search per lane.
   */
  ...
  public static void compareBool(MemorySegment a, MemorySegment b, CompareOp op, MemorySegment out, int n) {
    int words = Bitmap.wordsFor(n);
    for (int w = 0; w < words; w++) {
  ...
  public static void compareBoolScalar(MemorySegment a, boolean value, CompareOp op, MemorySegment out, int n) {
    int words = Bitmap.wordsFor(n);
    long y = value ? -1L : 0L;
    for (int w = 0; w < words; w++) {
      long x = Bitmap.wordAt(a, w, n);
