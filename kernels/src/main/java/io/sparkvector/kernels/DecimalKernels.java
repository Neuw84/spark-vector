...
/**
 * Kernels for decimals of at most 18 digits, held as unscaled {@code INT64} lanes; the scale lives
 * in the caller's logical type. Additions, subtractions and multiplications whose result precision
 * (in Spark's sense: {@code max(p1-s1, p2-s2) + max(s1,s2) + 1}, resp. {@code p1+p2+1}) stays within
 * 18 digits cannot overflow, so they are plain {@link ArithKernels} long operations after the
 * operands have been brought to a common scale with {@link #mulPow10}. What this class adds is:
 *
 * <ul>
 *   <li>rescaling: multiply by a power of ten (SIMD) or divide by one with Spark's
 *       {@code ROUND_HALF_UP} (scalar; integer division has no lanes);
 *   <li>range checks against a precision bound, producing a bitmap of the offending rows for the
 *       caller to turn into nulls (legacy mode) or an error (ANSI);
 *   <li>division, which Spark defines as {@code BigDecimal.divide(divisor, 39, HALF_UP)} followed
 *       by {@code setScale(resultScale, HALF_UP)}. When the scaled dividend fits in a long, one
 *       integer division with a half-up correction gives the same result (with a divisor below
 *       10^18 the two roundings cannot disagree); otherwise the row takes the {@link BigDecimal}
 *       path that reproduces Spark exactly;
 *   <li>conversions to and from doubles and integers with Spark's rounding.
 * </ul>
 *
 * <p>Validity is never touched here: the caller shares or combines the operand bitmaps and folds
 * the range-check bitmaps in.
 */
public final class DecimalKernels {
  ...
  /** Largest magnitude a decimal of {@code precision} digits can hold: {@code 10^p - 1}. */
  ...
  /** {@code out[i] = a[i] * 10^power} (INT64 lanes; the caller guarantees no overflow). */
  public static void mulPow10(VectorBuffers a, int power, MemorySegment out) {
    requireInt64(a);
    if (power == 0) {
      MemorySegment.copy(a.data(), 0, out, 0, (long) a.length() << 3);
      return;
    }
  ...
  /**
   * {@code out[i] = round_half_up(a[i] / 10^power)}, rounding half away from zero like Spark's
   * {@code Decimal.changePrecision} with {@code ROUND_HALF_UP}.
   */
  public static void divPow10HalfUp(VectorBuffers a, int power, MemorySegment out) {
    requireInt64(a);
    int n = a.length();
    if (power == 0) {
      MemorySegment.copy(a.data(), 0, out, 0, (long) n << 3);
      return;
    }
    long p = POW10[power];
  ...
  /**
   * Sets bit {@code i} of {@code outBits} where {@code |a[i]| > bound} (INT64 lanes). Bits of
   * other rows are cleared. Returns the number of rows out of range.
   */
  ...
  /**
   * Decimal division {@code a(s1) / b(s2)} into a result of {@code resultScale}. Rows whose divisor
   * is zero produce 0 and must be masked by the caller (see {@link CompareKernels}); rows whose
   * quotient does not fit {@code resultPrecision} digits are flagged in {@code overflowBits} (all
  ...
  public static int divide(
    ...
    requireInt64(b);
    int n = a.length();
    Div div = new Div(s1, s2, resultScale, resultPrecision, n, out, overflowBits);
  ...
  public static int divideScalar(
      VectorBuffers a, long divisor, int s1, int s2, int resultScale, int resultPrecision,
      MemorySegment out, MemorySegment overflowBits) {
    requireInt64(a);
    int n = a.length();
    Div div = new Div(s1, s2, resultScale, resultPrecision, n, out, overflowBits);
  ...
  public static int scalarDivide(
      long dividend, VectorBuffers b, int s1, int s2, int resultScale, int resultPrecision,
      MemorySegment out, MemorySegment overflowBits) {
    requireInt64(b);
    int n = b.length();
    Div div = new Div(s1, s2, resultScale, resultPrecision, n, out, overflowBits);
  ...
  private static final class Div {
    ...
    Div(int s1, int s2, int resultScale, int resultPrecision, int n, MemorySegment out, MemorySegment overflowBits) {
      this.s1 = s1;
      this.s2 = s2;
    ...
    /** Spark's two roundings, exactly: divide at scale 39 half-up, then to the result scale. */
    private long divideBig(long a, long b) {
      BigDecimal q =
          new BigDecimal(BigInteger.valueOf(a), s1)
              .divide(new BigDecimal(BigInteger.valueOf(b), s2), 39, RoundingMode.HALF_UP)
  ...
  /**
   * Decimal to double, exactly as {@code BigDecimal.doubleValue()}: for unscaled values below 2^53
   * the correctly rounded IEEE division {@code v / 10^scale} is the same number; larger values take
   * the {@link BigDecimal} path.
   */
  ...
  /**
   * Double to decimal with Spark's semantics ({@code Decimal(double)} goes through {@code
   * Double.toString}, then {@code changePrecision} rounds half up). NaN, infinities and values out
   * of range are flagged in {@code invalidBits} (other bits cleared) and produce 0. Returns the
   * number of flagged rows.
   */
  public static int fromDouble(VectorBuffers a, int precision, int scale, MemorySegment out, MemorySegment invalidBits) {
  ...
  /**
   * Integer (INT32 or INT64) to decimal: widens and multiplies by {@code 10^scale}. Rows whose
   * value would exceed {@code precision} digits are flagged in {@code invalidBits} (other bits
   * cleared) and produce 0; the check happens before the multiplication so it cannot wrap. Returns
  ...
  /**
   * Decimal to a long, truncating toward zero ({@code BigDecimal.longValue()} and Spark's ANSI
   * {@code roundToLong} agree for 18-digit values, which always fit).
   */
  ...
  /**
   * Decimal to an int, truncating toward zero. Values outside the int range are flagged in {@code
   * invalidBits} (other bits cleared) and written wrapped, which is what Spark's legacy cast does;
   * ANSI callers raise instead. Returns the number of flagged rows.
   */
  public static int toInt(VectorBuffers a, int scale, MemorySegment out, MemorySegment invalidBits) {
    requireInt64(a);
    int n = a.length();
    long p = POW10[scale];
    Bitmap.fill(invalidBits, n, false);
