...
/**
 * Casts to and from the {@link VecType#DECIMAL128} lane with Spark's {@code Cast} semantics (#258).
 * Every value is an unscaled integer at its declared scale. Rows that do not fit the target are
 * reported in an {@code invalid} bitmap (null in legacy mode, an error in ANSI mode -- the caller
 * decides) and their data is unspecified. Like {@link WideDecimalKernels}, the loops are scalar over
 * the limbs with the exact {@code BigInteger} / {@code BigDecimal} path wherever a value leaves 64
 * bits or the operation rounds.
 */
public final class WideDecimalCastKernels {
  ...
  /** Spark's {@code Decimal.changePrecision}: rescale (half up when digits drop), then the precision check. */
  private static BigInteger rescaled(BigInteger unscaled, int fromScale, int toScale) {
    if (toScale == fromScale) return unscaled;
    if (toScale > fromScale) return unscaled.multiply(WideDecimalKernels.pow10(toScale - fromScale));
  ...
  /**
   * A decimal (DECIMAL128 or INT64 lane) to a DECIMAL128 of another type. On the limbs when the value
   * fits a long and the scale grows within a long; the exact path otherwise.
   */
  public static void toWide(VectorBuffers a, int fromScale, int toScale, int toPrecision, int n, MemorySegment out, MemorySegment invalid) {
    int k = toScale - fromScale;
    boolean wide = a.type() == VecType.DECIMAL128;
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
      if (k >= 0 && k <= 18) {
        ...
        if (WideDecimalKernels.fitsLong(hi, lo)) {
          ...
          if (Math.multiplyHigh(lo, p) == (r >> 63)) {
            if (WideDecimalKernels.fitsPrecision(r >> 63, r, toPrecision)) Decimal128.set(out, i, r >> 63, r);
            else Bitmap.set(invalid, i);
            continue;
          }
        }
      }
      BigInteger u = rescaled(big(a, i), fromScale, toScale);
      if (fits(u, toPrecision)) Decimal128.set(out, i, Decimal128.hiOf(u), Decimal128.loOf(u));
      else Bitmap.set(invalid, i);
    }
  }

  /** A DECIMAL128 to a decimal of at most 18 digits (the INT64 lane): rescale, then the precision check guarantees it fits a long. */
  public static void toNarrow(VectorBuffers a, int fromScale, int toScale, int toPrecision, int n, MemorySegment out, MemorySegment invalid) {
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
      BigInteger u = rescaled(a.getDecimal128(i), fromScale, toScale);
      if (fits(u, toPrecision)) out.setAtIndex(VectorBuffers.LE_LONG, i, u.longValue());
      else Bitmap.set(invalid, i);
    }
  }

  /** An int or long column to a DECIMAL128: {@code value * 10^scale}, checked against the precision. */
  public static void fromIntegral(VectorBuffers a, int toScale, int toPrecision, int n, MemorySegment out, MemorySegment invalid) {
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
  ...
  public static void fromDouble(VectorBuffers a, int toScale, int toPrecision, int n, MemorySegment out, MemorySegment invalid) {
    for (int i = 0; i < n; i++) {
      ...
      BigInteger u = BigDecimal.valueOf(d).setScale(toScale, RoundingMode.HALF_UP).unscaledValue();
      if (fits(u, toPrecision)) Decimal128.set(out, i, Decimal128.hiOf(u), Decimal128.loOf(u));
      else Bitmap.set(invalid, i);
    }
  }

  /** A DECIMAL128 to double: {@code BigDecimal.doubleValue()}, which is what Spark's {@code Decimal.toDouble} does. */
  public static void toDouble(VectorBuffers a, int scale, int n, MemorySegment out) {
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
  ...
  /**
   * A DECIMAL128 to long, truncating toward zero as Spark's {@code Decimal.toLong} does. A value outside
   * the long range is invalid (Spark raises in ANSI mode and wraps -- the low 64 bits -- in legacy mode;
   * the wrapped value is written so the legacy caller can keep it).
   */
  public static void toLong(VectorBuffers a, int scale, int n, MemorySegment out, MemorySegment invalid) {
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
      BigInteger t = scale == 0 ? a.getDecimal128(i) : a.getDecimal128(i).divide(WideDecimalKernels.pow10(scale));
      if (t.bitLength() >= 64) Bitmap.set(invalid, i);
      out.setAtIndex(VectorBuffers.LE_LONG, i, t.longValue());
    }
  }

  /** A DECIMAL128 to int: the long above, then the int range; the wrapped value is written. */
  public static void toInt(VectorBuffers a, int scale, int n, MemorySegment out, MemorySegment invalid) {
    for (int i = 0; i < n; i++) {
      if (isNull(a, i)) continue;
      BigInteger t = scale == 0 ? a.getDecimal128(i) : a.getDecimal128(i).divide(WideDecimalKernels.pow10(scale));
      if (t.bitLength() >= 32) Bitmap.set(invalid, i);
  ...
  /** Two's-complement negation on the limbs. A decimal's range is symmetric, so a valid value never overflows. */
  ...
  /** |value| on the limbs: the value when the high limb is non-negative, its negation otherwise. */
