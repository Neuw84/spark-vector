...
/**
 * String comparisons in Spark's default {@code UTF8_BINARY} order: unsigned byte-wise lexicographic,
 * shorter prefix first -- exactly {@code UTF8String.compareTo}, which is also code-point order. One
 * result bit per row; null lanes get arbitrary bits, the caller shares the validity.
 *
 * <p>Column vs literal: a dictionary-encoded column is compared once per dictionary entry and the
 * verdicts gathered through the indices, so a Parquet dictionary page costs one compare per distinct
 * value. A plain column is compared row by row; equality checks the length first and then finds
 * the first mismatching byte with {@link MemorySegment#mismatch}, ordered compares decide on that
 * byte (or on the lengths when one string is a prefix of the other). 64-row blocks whose
 * {@code active} word is zero are skipped, like the fixed-width kernels.
 *
 * <p>Column vs column resolves each side's bytes (through the dictionary where there is one) and
 * compares the two ranges the same way.
 */
public final class StringCompareKernels {
  private StringCompareKernels() {}

  /** {@code a <op> s}; {@code s} is the literal's UTF-8 bytes. {@code active} may be null. */
  public static void compareScalar(VectorBuffers a, byte[] s, CompareOp op, MemorySegment active, MemorySegment out) {
  ...
  /** {@code a <op> b}; both UTF8 of the same length, either side dictionary encoded or plain. */
  public static void compare(VectorBuffers a, VectorBuffers b, CompareOp op, MemorySegment active, MemorySegment out) {
  ...
  /**
   * Unsigned byte-wise comparison of {@code x[xs, xe)} against {@code y[ys, ye)}: negative, zero or
   * positive like {@code Comparable.compareTo}.
   */
  public static int compareBytes(MemorySegment x, long xs, long xe, MemorySegment y, long ys, long ye) {
    long lx = xe - xs, ly = ye - ys;
    long common = Math.min(lx, ly);
    long at = MemorySegment.mismatch(x, xs, xs + common, y, ys, ys + common);
    if (at >= 0) {
  ...
  /** Per-row byte range of one operand, resolved through its dictionary when it has one. */
