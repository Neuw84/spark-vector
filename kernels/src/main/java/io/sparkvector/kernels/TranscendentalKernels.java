...
/**
 * Transcendental and trigonometric functions over FLOAT64 lanes, one scalar call per lane.
 *
 * <p>The call is exactly the one Spark's generated code makes -- {@code java.lang.Math} for the
 * trigonometric and hyperbolic functions, {@code cbrt}, {@code degrees}/{@code radians}, {@code atan2}
 * and {@code hypot}; {@code java.lang.StrictMath} for {@code exp}/{@code expm1}, the log family and
 * {@code pow}; Spark's own formulas for the inverse hyperbolics -- so a lane is bit-identical to
 * Spark's value and no ulp tolerance enters. The Vector API's {@code EXP}/{@code LOG}/{@code SIN}...
 * operators are 1-2 ulp off {@code StrictMath} and are deliberately not used here (a measured
 * follow-up, see the expression matrix). Nothing here touches validity: the caller nulls the lanes
 * Spark nulls -- the log family at or below its asymptote ({@link #asymptote}), {@code log(base, x)}
 * for a non-positive operand ({@link #positive}); every other domain edge is {@code Math}'s NaN.
 */
public final class TranscendentalKernels {
  ...
  public enum Fn {
    EXP, EXPM1, LOG, LOG2, LOG10, LOG1P, CBRT,
    SIN, COS, TAN, ASIN, ACOS, ATAN, SINH, COSH, TANH, ASINH, ACOSH, ATANH,
    COT, SEC, CSC, DEGREES, RADIANS
  }

  /** The binary functions. */
  public enum Fn2 { POW, ATAN2, HYPOT, LOG_BASE }
  ...
  /**
   * The value at or below which Spark's {@code UnaryLogExpression} is null instead of NaN or
   * -Infinity: 0 for the logarithms, -1 for {@code log1p}; NaN for a function without one.
   */
  ...
  /** {@code out[i] = fn(a[i])} over a FLOAT64 lane. Null lanes hold arbitrary values. */
  ...
  public static void binary(Fn2 fn, VectorBuffers a, VectorBuffers b, MemorySegment out) {
    requireDouble(a, fn.name());
  ...
  public static void binaryScalar(Fn2 fn, VectorBuffers a, double s, boolean reversed, MemorySegment out) {
    requireDouble(a, fn.name());
    int n = a.length();
    MemorySegment da = a.data();
    for (int i = 0; i < n; i++) {
      double x = da.getAtIndex(VectorBuffers.LE_DOUBLE, i);
      out.setAtIndex(VectorBuffers.LE_DOUBLE, i, reversed ? apply(fn, s, x) : apply(fn, x, s));
    }
  }

  /**
   * Sets bit {@code i} of {@code outBits} where {@code !(a[i] <= threshold)} -- Spark's domain test,
   * which keeps NaN (NaN is not {@code <=} anything, so {@code log(NaN)} is NaN, not null) -- and
   * clears it otherwise.
   */
