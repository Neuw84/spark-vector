...
/**
 * Accumulates the rows of many batches of one column into a single Arrow-layout column in native
 * memory, for blocking operators (sort, hash-join build side) that must see a whole partition.
 * Batches are appended whole or through a selection bitmap; dictionary-encoded strings are decoded
 * on the way in, since the dictionaries of different batches are unrelated.
 *
 * <p>Buffers grow by doubling inside the given {@link Arena}; a shared arena is expected, so the
 * abandoned smaller buffers are released with everything else when the operator closes.
 */
public final class ColumnBuilder {
  ...
  /** Appends the rows of {@code in} set in {@code selection} ({@code null} for all rows). */
    ...
    switch (type) {
      ...
      case BOOL -> {
        if (selection == null) {
          ...
          int o = start;
          for (int w = 0, words = Bitmap.wordsFor(in.length()); w < words; w++) {
      ...
      default -> {
        ...
        if (selection == null) {
          MemorySegment.copy(in.data(), 0, data, (long) start * width, (long) count * width);
        } else {
  ...
  private void appendValidity(VectorBuffers in, MemorySegment selection, int count, int start) {
    if (validity == null) {
      return;
    }
    if (!in.hasNulls()) {
  ...
  private void appendUtf8(VectorBuffers in, MemorySegment selection, int count, int start) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (!in.isNull(i)) {
        ...
        MemorySegment.copy(src, ValueLayout.JAVA_BYTE, srcStart, data, ValueLayout.JAVA_BYTE, pos, len);
        pos += len;
      }
      o++;
    }
