...
class BitKernelsTest {
  ...
  private static long ref(BitOp op, long x, long y, boolean isInt) {
    return switch (op) {
      case AND -> x & y;
      case OR -> x | y;
      case XOR -> x ^ y;
  ...
  void binaryColumnColumnAndColumnScalar() {
    try (Arena arena = Arena.ofConfined()) {
      ...
      xi[0] = Integer.MIN_VALUE; xi[1] = -1; xl[0] = Long.MIN_VALUE; xl[1] = -1L;
      ...
      for (BitOp op : BitOp.values()) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          assertEquals((int) ref(op, xi[i], yI, true), outi.getAtIndex(VectorBuffers.LE_INT, i), op + " int row " + i);
          assertEquals(ref(op, xl[i], yL, false), outl.getAtIndex(VectorBuffers.LE_LONG, i), op + " long row " + i);
        }
        for (long s : new long[] {0, 1, 5, 31, 32, 33, 63, 64, -1, 0xF0F0F0F0L, -6148914691236517206L}) {
          BitKernels.binaryScalar(op, ai, s, outi);
          BitKernels.binaryScalar(op, al, s, outl);
          for (int i = 0; i < n; i++) {
            assertEquals((int) ref(op, xi[i], (int) s, true), outi.getAtIndex(VectorBuffers.LE_INT, i), op + " int scalar " + s + " row " + i);
            assertEquals(ref(op, xl[i], op.isShift() ? (int) s : s, false), outl.getAtIndex(VectorBuffers.LE_LONG, i), op + " long scalar " + s + " row " + i);
          }
        }
        if (op.isShift()) {
          ...
          for (int i = 0; i < n; i++) {
            assertEquals((int) ref(op, -12345, amounts[i], true), outi.getAtIndex(VectorBuffers.LE_INT, i), op + " literal value int row " + i);
            assertEquals(ref(op, Long.MIN_VALUE + 7, amounts[i], false), outl.getAtIndex(VectorBuffers.LE_LONG, i), op + " literal value long row " + i);
          }
        }
      }
  ...
  void notAndBitCount() {
    try (Arena arena = Arena.ofConfined()) {
      int[] xi = {0, 1, -1, Integer.MIN_VALUE, Integer.MAX_VALUE, 0x0F0F0F0F, 12345, -12345, 7, 8, 9, 10, 11};
      long[] xl = {0L, 1L, -1L, Long.MIN_VALUE, Long.MAX_VALUE, 0x0F0F0F0F0F0F0F0FL, 12345L, -12345L, 7, 8, 9, 10, 11};
