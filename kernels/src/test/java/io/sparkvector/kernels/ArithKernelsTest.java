...
class ArithKernelsTest {
  ...
  private static void assertSameData(VecType t, MemorySegment exp, MemorySegment act, int n, String what) {
    for (int i = 0; i < n; i++) {
      switch (t) {
        case INT32 -> assertEquals(exp.get(VectorBuffers.LE_INT, (long) i << 2), act.get(VectorBuffers.LE_INT, (long) i << 2), what + " @" + i);
        case INT64 -> assertEquals(exp.get(VectorBuffers.LE_LONG, (long) i << 3), act.get(VectorBuffers.LE_LONG, (long) i << 3), what + " @" + i);
    ...
    for (int n : TestData.LENGTHS) {
      try (Arena arena = Arena.ofConfined()) {
        ...
        for (int t = 0; t < 3; t++) {
          ...
          assertSameData(as[t].type(), exp, act, n, as[t].type() + " " + op);
        }
      }
    }
  }
  ...
  void scalarOperandsMatchReference(ArithOp op) {
    ...
    for (int n : TestData.LENGTHS) {
      try (Arena arena = Arena.ofConfined()) {
        VectorBuffers[] as = {wideInts(arena, rnd, n), wideLongs(arena, rnd, n), TestData.doubles(arena, rnd, n, null)};
        Number[] scalars = {rnd.nextBoolean() ? Integer.MIN_VALUE : rnd.nextInt(-9, 9), rnd.nextLong(-9, 9), rnd.nextInt(-3, 4) * 0.5};
        for (int t = 0; t < 3; t++) {
          if (op == ArithOp.DIV && as[t].type() != VecType.FLOAT64) {
            continue;
          }
          ...
          assertSameData(as[t].type(), exp, act, n, as[t].type() + " col " + op + " scalar");

          ScalarReference.scalarArith(op, scalars[t], as[t], exp);
          ArithKernels.scalarArith(op, scalars[t], as[t], act);
          assertSameData(as[t].type(), exp, act, n, as[t].type() + " scalar " + op + " col");
        }
      }
    }
  }
    ...
    for (int n : TestData.LENGTHS) {
      try (Arena arena = Arena.ofConfined()) {
        ...
        for (VectorBuffers a : as) {
          ...
          assertSameData(a.type(), exp, act, n, a.type() + " neg");
        }
      }
    }
  }
  ...
  void divisionByZeroProducesInfOrNanValuesForCallerToMaskOut() {
    try (Arena arena = Arena.ofConfined()) {
      VectorBuffers a = ArrowLayout.ofDoubles(arena, new double[] {1.0, -1.0, 0.0, 5.0}, null);
      VectorBuffers b = ArrowLayout.ofDoubles(arena, new double[] {0.0, 0.0, 0.0, -0.0}, null);
