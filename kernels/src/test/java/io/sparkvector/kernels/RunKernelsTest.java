...
/** Run boundaries on sorted keys against a scalar reference built on {@link RunMerge#compareKeys}. */
class RunKernelsTest {

  private static final int[] SIZES = {0, 1, 2, 63, 64, 65, 127, 128, 1000, 4097};
    ...
    try (Arena arena = Arena.ofConfined()) {
      for (int n : SIZES) {
        for (boolean nullsFirst : new boolean[] {true, false}) {
          ...
          check(arena, sortedInts(arena, rnd, n, nulls, nullsFirst), n, "int32 n=" + n);
          check(arena, sortedLongs(arena, rnd, n, nulls, nullsFirst), n, "int64 n=" + n);
          check(arena, sortedDoubles(arena, rnd, n, nulls, nullsFirst), n, "float64 n=" + n);
          check(arena, sortedStrings(arena, rnd, n, nulls, nullsFirst, false), n, "utf8 n=" + n);
          check(arena, sortedStrings(arena, rnd, n, nulls, nullsFirst, true), n, "dictionary n=" + n);
          check(arena, sortedBooleans(arena, rnd, n, nulls, nullsFirst), n, "bool n=" + n);
        }
      }
    }
  }
  ...
  void twoKeysAndTheDoubleTotalOrder() {
    try (Arena arena = Arena.ofConfined()) {
      ...
      double[] d = {-1.0, -0.0, 0.0, 0.0, 2.5, Double.NaN, Double.NaN};
      VectorBuffers key = ArrowLayout.ofDoubles(arena, d, null);
      assertArrayEquals(new int[] {0, 1, 4, 5, 7}, starts(new VectorBuffers[] {key}, d.length));
      ...
      assertArrayEquals(new int[] {0, 2, 3, 5, 6}, starts(keys, a.length));
  ...
  private static void check(Arena arena, VectorBuffers key, int n, String what) {
  ...
  private static int[] referenceStarts(VectorBuffers[] keys, int n) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int k = 0; k < keys.length && !boundary; k++) {
        boundary = RunMerge.compareKeys(keys[k], i, keys[k], i - 1, false, true) != 0;
      }
      if (boundary) {
        tmp[o++] = i;
      }
  ...
  private static VectorBuffers sortedInts(Arena arena, Random rnd, int n, int nulls, boolean nullsFirst) {
    int[] v = new int[n];
    for (int i = 0; i < n; i++) {
      v[i] = rnd.nextInt(7) - 3;
    }
  ...
  private static VectorBuffers sortedLongs(Arena arena, Random rnd, int n, int nulls, boolean nullsFirst) {
    long[] v = new long[n];
    for (int i = 0; i < n; i++) {
  ...
  private static VectorBuffers sortedDoubles(Arena arena, Random rnd, int n, int nulls, boolean nullsFirst) {
    double[] pool = {-2.5, -0.0, 0.0, 1.25, Double.POSITIVE_INFINITY, Double.NaN};
    ...
    Arrays.sort(boxed, (x, y) -> Long.compareUnsigned(SortKernels.doubleKey(x), SortKernels.doubleKey(y)));
    for (int i = 0; i < n; i++) {
      v[i] = boxed[i];
    }
  ...
  private static VectorBuffers sortedStrings(Arena arena, Random rnd, int n, int nulls, boolean nullsFirst, boolean dictionary) {
    String[] pool = {"", "a", "ab", "abc", "b", "ba", "\u00e9", "a longer value than eight bytes"};
    String[] v = new String[n];
    for (int i = 0; i < n; i++) {
      v[i] = pool[rnd.nextInt(pool.length)];
    }
    Arrays.sort(v, (x, y) -> SortReference.compareBytes(x.getBytes(StandardCharsets.UTF_8), y.getBytes(StandardCharsets.UTF_8)));
    boolean[] b = nullBlock(n, nulls, nullsFirst);
    if (!dictionary) {
      if (b != null) {
        for (int i = 0; i < n; i++) {
  ...
  private static VectorBuffers sortedBooleans(Arena arena, Random rnd, int n, int nulls, boolean nullsFirst) {
