...
- Expressions compile to a small `VectorExpr` tree (`ColumnRef`, `LiteralExpr`, `CompareExpr`
  ...
  `LIKE` shapes `LikeSimplification` rewrites, via `StringMatchKernels`), the string measures in `StringLengthExprs.scala` (`StringMeasureExpr` for length/octet_length/bit_length/ascii, once per dictionary entry via `StringLengthKernels.perEntry`, and `ChrExpr`), the string writers in
...
Current counts: 142 kernel tests, 176 Spark tests (149 without the Comet and Iceberg profiles;
