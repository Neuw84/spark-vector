...
| `HashAggregateExec` in `PartialMerge` / `Complete` mode, keys-only (distinct-style) aggregates, `count(distinct)` | #7 | Only `Partial` and `Final` modes with at least one function are accepted; `Expand` below them is Supported above |
