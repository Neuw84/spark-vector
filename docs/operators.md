...
| Keys-only (distinct-style) aggregates and `count(distinct)` itself | follow-up of #7 | Every mode is accepted now, but an operator without functions and a `DISTINCT` aggregate expression still fall back; `Expand` below them is Supported above |
