...
- The final `Sort` and the row conversion above our Final aggregate are Spark's. Since phase 3
  the Comet-scan configurations are ahead of Comet end to end on Q1, so these no longer decide the
  comparison; they are still the only non-vectorized operators left in that plan.
