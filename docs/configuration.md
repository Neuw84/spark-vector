...
| `spark.vector.shuffle.aqe.sparkCompressionRatio` | `2.5` | double | The compression `mapSizeScaling` expects of Spark's shuffle (uncompressed `UnsafeRow` bytes over bytes on disk; TPC-DS 1 TB: 2.57 over the run, 1.8-3.4 per query). `0` or negative assumes Spark's shuffle compresses as well as ours. |
