...
| `sqrt` | FLOAT64 -> FLOAT64 | `Math.sqrt` per lane (Spark's); a negative argument is NaN. Landed for the statistical aggregates' result expressions; the rest of #35 is still open |
...
| Transcendental and trigonometric functions beyond `sqrt` | #35 |
