...
| `+` `-` `*` on integers | INT32, INT64 | Same-typed operands (`arithmetic operands differ`). **Legacy mode only**: in ANSI mode (Spark's default) integer arithmetic falls back with `ANSI integer arithmetic (overflow checks) not supported` (#8). `try_*`: `try_* arithmetic not supported` (#47) |
...
| Unary minus | INT32, INT64, FLOAT64, decimal(<=18) | ANSI integer negation falls back (`ANSI integer negation (overflow check) not supported`, #8); doubles and decimals never overflow here |
