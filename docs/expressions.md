...
| any other function | -- | `unsupported aggregate function <Class>: <sql>` (`stddev`/`variance`: #46; `collect_*`, `percentile_*` and other `TypedImperativeAggregate`s are planned by Spark as `ObjectHashAggregateExec`, which is not converted) |
