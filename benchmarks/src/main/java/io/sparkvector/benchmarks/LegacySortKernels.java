...
/**
 * The index sort before #285, kept only as the baseline {@link SortBenchmark} measures the radix
 * kernel against: one {@code Arrays.sort(long[])} per 32-bit key pass. Not used by the plugin. Its
...
final class LegacySortKernels {
   ...
   * direction {@code ascending[c]} and nulls placed according to {@code nullsFirst[c]}; the first
   * key is the most significant. Stable: rows equal on every key keep their input order.
   */
  public static int[] sortIndices(VectorBuffers[] keys, boolean[] ascending, boolean[] nullsFirst, int n) {
    int[] order = new int[n];
    for (int i = 0; i < n; i++) {
      order[i] = i;
    }
  ...
  /**
   * Total order on doubles as an unsigned 64-bit key, matching Spark's comparison: {@code -0.0}
   * and {@code 0.0} are equal, NaN is greater than every other value and equal to itself.
   */
  ...
  /**
   * Dense rank of every row's string (nulls get 0, which the null pass separates out anyway):
   * rows are merge-sorted by bytes, then equal neighbours share a rank.
   */
  ...
  private static void mergeSort(int[] a, int[] tmp, int from, int to, VectorBuffers k) {
    if (to - from < 2) {
      return;
    }
    int mid = (from + to) >>> 1;
