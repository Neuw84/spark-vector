...
/**
 * The crossing cost between our batches and Comet's (#279), both directions, so a candidate swap of
 * one operator for Comet's can be judged as "operator delta minus twice the crossing". Into Comet:
 * {@link CometBatchBridge#convert} -- our Arrow vectors exported through the C Data interface
 * (buffers pointed at, retained; a dictionary decoded; an INT64 decimal widened to 128 bits) and
 * imported by Comet's {@code ArrowImporter}, then the imported vectors closed (the release callback).
 * Back: {@link ColumnVectorAdapters#adapt} over the Comet vectors that conversion produced, through
 * the registered {@link CometVectorAdapter} (zero-copy but for large strings and, before #257,
 * 128-bit decimals). The unit is microseconds per batch; the docs quote ns per row per column.
 *
 * <pre>
...
public class CrossingBenchmark {
  ...
  /**
   * INT64 (no validity buffer), INT64_NULLS (10% nulls), FLOAT64, UTF8 (12-40 bytes), UTF8_DICT
   * (dictionary of 1000 values, decoded when crossing into Comet), DECIMAL64 (decimal(12,2) on the INT64
   * lane, widened to 128 bits when crossing into Comet).
   */
  ...
  /** Ours into Comet: export through C Data, Comet's import, then the imported vectors released. */
  @Benchmark
