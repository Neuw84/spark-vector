...
/**
 * Wide decimals (#258): the two-limb DECIMAL128 kernels against the per-row {@code BigDecimal} loop
 * Spark's row path runs, on {@code decimal(38,10)} columns. {@code magnitude} picks the values: {@code
 * long} -- unscaled values that fit a long (TPC-H-sized amounts declared wide, the common case), where
 * the kernels stay on the limbs; {@code wide} -- unscaled values of 30-37 digits, where {@code +} still
 * runs on the limbs and {@code *} takes the exact path on every row (the product leaves 128 bits).
 * Division is {@code BigDecimal} by construction on both sides, so its two numbers show only the
 * columnar overhead.
 */
...
public class WideDecimalBenchmark {
  ...
  MemorySegment zero;
  MemorySegment sel;
  /** Sink for the reference loops; the results are summed into it so the JIT keeps them. */
  long sink;
  ...
  public void setup() {
    ...
    for (int i = 0; i < N; i++) {
      if ("long".equals(magnitude)) {
        ...
        if (rnd.nextBoolean()) x[i] = x[i].negate();
      }
    }
  ...
  public MemorySegment addKernel() {
    ...
    WideDecimalKernels.addSub(
        WideDecimalKernels.Operand.of(a, 0), WideDecimalKernels.Operand.of(b, 0), false, SCALE, SCALE, PRECISION, N, out, overflow);
    return out;
  }
  ...
  public long addReference() {
    ...
    for (int i = 0; i < N; i++) {
      ...
      if (r.precision() > PRECISION) s++;
      else s += r.unscaledValue().bitLength();
    }
    return sink = s;
  }
  ...
  public MemorySegment mulKernel() {
    ...
    WideDecimalKernels.mul(
        WideDecimalKernels.Operand.of(a, 0), WideDecimalKernels.Operand.of(b, 0), 2 * SCALE, SCALE, PRECISION, N, out, overflow);
    return out;
  }
  ...
  public MemorySegment mulExactScaleKernel() {
    Bitmap.fill(overflow, N, false);
    WideDecimalKernels.mul(
        WideDecimalKernels.Operand.of(a, 0), WideDecimalKernels.Operand.of(b, 0), 4, 4, PRECISION, N, out, overflow);
    return out;
  }
  ...
  public MemorySegment divKernel() {
    ...
    WideDecimalKernels.divide(
        WideDecimalKernels.Operand.of(a, 0), SCALE, WideDecimalKernels.Operand.of(b, 0), SCALE, SCALE, PRECISION, N, out, overflow, zero);
    return out;
  }
  ...
  public long compareScalarReference() {
    long s = 0;
    for (int i = 0; i < N; i++) if (ra[i].compareTo(scalarDec) < 0) s++;
    return sink = s;
  }
  ...
  public long compareColumnReference() {
    long s = 0;
    for (int i = 0; i < N; i++) if (ra[i].compareTo(rb[i]) < 0) s++;
    return sink = s;
  }
}
