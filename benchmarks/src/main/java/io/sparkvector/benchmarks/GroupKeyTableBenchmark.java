...
/**
 * Group-id assignment over one plain (non-dictionary) UTF8 key column, the shape Comet's native
 * scan hands over: `keyBytes` is the value length (8 fits the packed fast path, 24 is a nation or
 * region name), `distinct` the cardinality (25 nations, 10k a wide key that still fits the plain
 * dictionary cap, 200k one that overflows it and takes the hashing path).
 */
...
public class GroupKeyTableBenchmark {
  ...
  /** Batches per table: the dictionary is built on the first and reused on the rest. */
  ...
  /** Assigns all batches into a fresh table; the reported rate is rows per millisecond. */
  @Benchmark
