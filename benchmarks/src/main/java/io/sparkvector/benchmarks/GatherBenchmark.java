...
/**
 * Gathering a plain UTF8 column by random indices, the join output path: the kernel (short strings
 * moved as long pairs through {@link ByteCopy}) against the same loop with one
 * {@link MemorySegment#copy} per string.
 */
...
public class GatherBenchmark {
  ...
  public int gather_kernel() {
    GatherKernels.gatherUtf8(in, idx, 0, N, outOffsets, outData, null);
