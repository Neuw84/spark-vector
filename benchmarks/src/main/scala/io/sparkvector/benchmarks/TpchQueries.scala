...
/**
 * The 22 TPC-H queries with the validation-run substitution parameters, as Spark SQL. Q1 and Q6 are
 * the texts the harness has always run (their checksums are in `benchmarks/results`); the other 20
 * are the texts of Spark's own TPC-H suite (`sql/core/src/test/resources/tpch`). Decimals are
 * doubles in the generated data (see `gen-tpch.sh`), so every literal is spelled as a plain number.
 */
object TpchQueries {
