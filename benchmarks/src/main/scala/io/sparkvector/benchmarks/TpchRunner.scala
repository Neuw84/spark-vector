...
object TpchRunner {
  ...
  /** The tables a query reads, from its text: every TPC-H table name is a distinct word. */
  def tablesOf(sql: String): Set[String] = {
    val words = sql.toLowerCase.split("[^a-z_]+").toSet
    Tables.filter(words.contains).toSet
  }

  final case class Args(
      config: String = "vector",
      data: String = "benchmarks/data/sf1",
      queries: Seq[String] = QueryOrder,
      iterations: Int = 5,
      warmup: Int = 2,
  ...
  private def run(args: Args): Unit = {
    ...
    try {
      ...
      val present = Tables.filter(t => new File(args.data, t).isDirectory)
      require(present.contains("lineitem"), s"${args.data} holds no lineitem/ directory (see gen-tpch.sh)")
      ...
      println(s"[tpch] config=${args.config} data=${args.data} tables=${present.mkString(",")} lineitem rows=$rowCount threads=${args.threads}")
      ...
      try {
        args.queries.foreach { q =>
          ...
          val missing = tablesOf(sql) -- present.toSet
          if (missing.nonEmpty) {
            println(s"[tpch] skipping $q: ${missing.toSeq.sorted.mkString(", ")} not generated (rerun gen-tpch.sh)")
          } else {
  ...
  /** TPC-H order (`q1`..`q22`) rather than lexical, with anything else after. */
  private def queryOrder(q: String): (Int, String) =
  ...
  private def report(dir: Path): Unit = {
    ...
    val datasets = rows.groupBy(_.dataset).toSeq.sortBy(_._2.head.lineitemRows).map { case (name, rs) =>
      ...
      DatasetReport(name, rs.head.lineitemRows, configs, latest.keys.map(_._2).toSeq.distinct.sortBy(queryOrder), latest)
    }
